...
class Jira:
    ...
    def create_issue(
        ...
        issue = self._create_issue_via_rest(issue_dict)
        LOGGER.info(
    ...
    def _create_issue_via_rest(self, issue_dict: dict[str, Any]) -> Issue:
        url = self.connection._get_url("issue")
        response = self.connection._session.post(
            url,
            json={"fields": issue_dict},
        ...
        return Issue(self.connection._options, self.connection._session, response.json())
    ...
    def add_labels_to_issue(self, issue_id_or_key: str, labels: list[str]) -> tuple[Issue, bool]:
        ...
        Returns:
            tuple[Issue, bool]: The issue and whether the labels were applied successfully.
        """
    ...
    def remove_labels_from_issue(self, issue_id_or_key: str, labels: list[str]) -> tuple[Issue, bool]:
        """
        Remove the given labels from a Jira issue, as identified by the issue's key or ID field value.

        Args:
            issue_id_or_key (str): The ID or key field value of the Jira Issue object to return.
            labels: list[str]: The list of labels to remove from the Jira issue.
        Returns:
            tuple[Issue, bool]: The issue and whether the labels were removed successfully.
        """
        issue = self.get_issue_by_id_or_key(issue_id_or_key)
        payload = {"update": {"labels": [{"remove": label} for label in labels]}}
        try:
            response = self.connection._session.put(
                issue.self,
                json=payload,
            ...
            LOGGER.error(
                f"Failed to remove labels {labels} from issue {issue_id_or_key}. Error: {error.text}",
            )
            if error.status_code in (400, 403):
                LOGGER.info(
                    "This may be caused by project configuration, missing permissions, or the Jira user."
